...
- **OpenAI Responses API** через тонкую абстракцию `LLMProvider` (`src/llm/provider.ts`); модели оркестратора и sub-agent задаются независимо в `.env` (по умолчанию `gpt-5.4-mini` / `gpt-5.4-nano`).

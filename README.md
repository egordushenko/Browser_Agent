...
**Тулы** — только атомарные примитивы: `navigate`, `query_dom`, `click`, `open_candidate`, `type`, `scroll`, `wait`, `read_page`, `screenshot`, `propose_selection`, `confirm_batch`, `execute_batch`, `ask_user`, `done`. Никаких задач-специфичных заготовок.

**Candidate registry (контракт perception → execution)** — LLM не владеет селекторами. Перцепция нумерует интерактивные элементы страницы (`c1`, `c2`, ...), selector-рецепты остаются внутри `CandidateRegistry` (`src/browser/candidate-registry.ts`); модель кликает/печатает только по `candidateId`. Навигация ограничена URL из задачи пользователя и href, реально увиденными в DOM, — угаданные внутренние пути блокируются (`src/agent/navigation-policy.ts`).

**DOM sub-agent** — единственное место, где живёт сырое представление страницы; в контекст оркестратора оно не попадает. Перцепция (`src/browser/perception.ts`): `page.ariaSnapshot()` как компактная основа + дайджест интерактивных элементов (видимые `a/button/input/...` с приоритетом селекторов `id` → `data-testid` → `name` → `aria-label` → `role+name` → `text`). Sub-agent (дешёвая модель) получает вопрос оркестратора и это представление, возвращает ответ + `candidateId` + confidence, а для списков — структурные объекты.

**Object memory (`src/agent/object-memory.ts`)** — извлечённые sub-agent'ом объекты (`email`, `product`, `vacancy`, `resume`) живут в памяти задачи со стабильными `objectId` и статусами `seen → opened → details_extracted → reviewed → selected → action_ready → action_done`. Память переживает переходы между страницами (page-scoped candidate ids обновляются при каждой экстракции) и подаётся оркестратору компактным дайджестом на каждом шаге.

**Checkpoints (`src/agent/checkpoints.ts`)** — код, а не промпт, гарантирует этапность: нельзя выбрать объект, который видели только в списке (`review_required`); деструктивное batch-действие не выполняется без подтверждения пользователя (`confirmation_required`); `done` заблокирован, пока предложенный/подтверждённый батч не завершён — либо модель честно передаёт `incomplete_reason`.

**Batch-действия** — `propose_selection` (выбрать N объектов с причиной) → `confirm_batch` (пользователь видит список и подтверждает y/n) → `execute_batch` (`delete`, `mark_spam`, `add_to_cart`, `apply`, `send_message`, `stop_before_payment`) выполняется по `objectId` через сохранённые action-контролы объектов. Отчёт строится из статусов памяти, а не из свободного текста модели.
...
- **Слабый контракт `LLM → selector` заменён на `LLM → candidateId / objectId`**: селекторы и URL создаёт код, модель только ссылается на них. Это убирает целый класс ошибок — выдуманные селекторы, ARIA-refs, угаданные внутренние URL, преждевременный `done` без выполнения батча.
